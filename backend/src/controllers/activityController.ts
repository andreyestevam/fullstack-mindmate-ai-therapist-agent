// Does the logic for logging user activities
...
export const logActivity = async (req: Request, res: Response, next: NextFunction) => {
    try{
        ...
        // This verification is possible because of the middleware since it would attach the user object to req.user
        if(!userId){
        ...
        next(error); // If any errors occurs it is passed to the error handler middleware
    }
};