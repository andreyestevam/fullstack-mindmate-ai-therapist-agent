// Handles the logic for creating mood entries.
...
export const createMood = async(req: Request, res: Response, next: NextFunction) => {
    try{
        ...
        // Possible to verify because of the authentication middleware.
        if(!userId){
        ...
        next(error); // Any errors are passed to the error handler middleware
    }
}