...
import { ArrowRight, Brain, HeartPulse, Lightbulb, Lock, MessageSquareHeart, Shield, Sparkles, Waves } from "lucide-react";
...
export default function Home() {
  // Define the emotional states and their corresponding colors
  const emotions = [
  ...
  ]; // This will be used for the slider (to define their mood and make their interaction feel emotionally responsive).
  ...
  const [emotion, setEmotion] = useState(50); // Default to "Peaceful"
  const [mounted, setMounted] = useState(false); // Used to trigger animations only after the component has loaded.
  ...
  // Now we need to track which emotion is currently selected!

  useEffect(() => {
    setMounted(true);
  }, []); // This effect runs only once when the component mounts. Otherwise the animation will not work properly.
  
  const currentEmotion = emotions.find((em) => Math.abs(emotion - em.value) < 15 ) || emotions[2]; // Default to "Peaceful" if no close match is found.

  // Now we begin our first main section the user will see once they get to the home page.
  return (<div className="flex flex-col min-h-screen overflow-hidden">
    <section className="relative min-h-[90-vh]
    mt-20 flex flex-col items-center justify-center py-12 px-4">
      {/* Glowing background effects */}
      <div className="absolute inset-0 -z-10 overflow-hidden">

      <div className={`absolute w-[500px] h-[500px] rounded-full blur-3xl top-0 -left-20
      transition-all duration-700 ease-in-out bg-gradient-to-r ${currentEmotion.color} to-transparent opacity-60`}/>
      ...
      <Ripple className="opacity-60"/> {/* Makes the circle breathing effect */}
      <motion.div
        ...
        {/* Enhanced badge with subtle animation */}
          ...
          {/* Main heading with smoother gradient */}
          ...
          <div className="space-y-2 text-center">
            ...
            <div className="flex justify-between items-center px-2">
              {emotions.map((em) => (
                <div
                  key = {em.value}
                  className = {`transition-all duration-500 ease-out cursor-pointer hover:scale-105 ${Math.abs(emotion - em.value) < 15 ? "opacity-100 scale-110 transform-gpu"
    ...
    {/* Features section (i.e. the benefits of using the service) */}
    <section className="relative py-20 px-4 overflow-hidden">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Display to show the feature items */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 relative">
          {features.map((feature, index) => (
            <motion.div
              ...
              viewport={{once:true}}
            >
              <Card className="group relative overflow-hidden border border-primary/10 hover:border-primary/20 transition-all duration-300
              h-[200px] bg-card/30 dark:bg-card/80 backdrop-blur-sm">
                <div
                  className={`absolute inset-0 bg-gradient-to-br ${feature.color} to-transparent opacity-0 group-hover:opacity-20 transition-opacity
                  duration-500 dark:group-hover:opacity-30`}
                />
                <CardHeader className="pb-4">
                  <div className="flex items-center gap-3">
                    <div className="p-2 rounded-xl bg-primary/10 dark:bg-primary/20 group-hover:bg-primary/20 dark:group-hover:bg-primary/30
                ...
                <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-primary/20 dark:via-primary/30 to-transparent
